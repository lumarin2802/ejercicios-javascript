 ...
 const ensaladaDeFrutas = ["manzana", "banana", "kiwi", "pera", "ciruela"]

 for (var i = 0; i < ensaladaDeFrutas.length; i++) {

     console.log("Las frutas que hay en la ensalada son: " + ensaladaDeFrutas)

 }

 //Ejercicio 2 Promedio Edad
 ...
 const verdaderosAmigos = []
 for (var i = 0; i < amigos.length; i++) {
     if (amigos[i].length <= 4)
         //const verdaderosAmigos = amigos.filter(amigos => amigos.length == 4);
         verdaderosAmigos.push(amigos[i])

 }
 console.log(verdaderosAmigos);

 //let verdaderosAmigos = amigo.filter(amigo => amigo.length == 4)
 ...
 console.log("Mi primera invocacion saludo()");
 saludo()
 ...
 function filtrarAmigos(arr) {
     ...
     for (var i = 0; i < amigos.length; i++) {
         if (arr[i].length === 4)
             ...
             verdaderosAmigos.push(arr[i])

     }
     return verdaderosAmigos;
 ...
 function tirarDado() {
     ...
     let aleatorio = (Math.floor(Math.random() * dado.length));
     console.log(dado[aleatorio]);
 }
 ...
 const toDoList = {
     ...
     dia: "Sábado",
     productos: ["yerba mate", "frutas", "carne"],
     'cantidad de productos': [4, 7, 10],
     estado: "pendiente",
